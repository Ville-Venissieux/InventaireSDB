...
class PdfGenerator {
    ...
    /**
     * Génère un export contenant les historiques des mouvements des articles pour un usager
     * @param type $pdf $pdf à générer
     * @param type $categorie filtre sur la catégorie
     * @param type $usager usager concerné
     */
    public static function generateHistoryExport($pdf, $categorie, $usager) {
        ...
        $pdf->SetTextColor(0, 110, 153);
        $pdf->SetDrawColor(255, 255, 255);
        $titre = 'Liste des mouvements d\'articles';
        if (isset($categorie)) {
            $titre .= ' de la catégorie ' . $categorie->getLibelle();
        }
        if (isset($usager)) {
        ...
        $pdf->Cell(60, 7, 'Nom', 0, 0, 'C', 1);
        $pdf->Cell(30, 7, 'Catégorie', 0, 0, 'C', 1);
        $pdf->Cell(20, 7, 'Date de prêt', 0, 0, 'C', 1);
        $pdf->Cell(20, 7, 'Date de retour', 0, 0, 'C', 1);
        $pdf->Cell(20, 7, 'Etat', 0, 0, 'C', 1);
        $pdf->Cell(120, 7, 'Commentaire', 0, 0, 'C', 1);
        $pdf->Ln();
        ...
        foreach ($usager->getPrets() as $pret) {
            
            $article = $pret->getArticle();

            //Dans le cas ou une catégorie est sélectionnée, on ne prend que les mouvements d'articles de cette catégorie
            if (!isset($categorie) or (isset($categorie)and $categorie->getId() == $article->getCategorie()->getId())  ) {
                ...
                $pdf->SetFont('helvetica', '', 8);
                $pdf->Cell(30, 7, $article->getCategorie() ? $article->getCategorie()->getLibelle() : '', 0, 0, 'C', 1);
                $pdf->Cell(20, 7, $pret->getDatePret() ? $pret->getDatePret()->format('Y-m-d') : '', 0, 0, 'C', 1);
                $pdf->Cell(20, 7, $pret->getDateRetour() ? $pret->getDateRetour()->format('Y-m-d') : '', 0, 0, 'C', 1);
                $pdf->Cell(20, 7, $article->getEtat() ? $article->getEtat()->getLibelle() : '', 0, 0, 'C', 1);
                //On tronque à 70 caractères pour ne pas dépaaser les limites de la cellule
                $pdf->Cell(120, 7, mb_strimwidth($article->getCommentaire(), 0, 70, "...", "UTF-8"), 0, 0, 'C', 1);
                $pdf->Ln();
